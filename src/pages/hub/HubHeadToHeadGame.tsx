...
import { getH2HGameLog } from '@/api/hub'
  ...
  return (
    <div>
      ...
      {isLoading || !data ? (
        ...
        <>
          <div className="flex items-center justify-between bg-surface border border-borderLow rounded-lg p-5 mb-5">
            ...
            <div className="bg-surface border border-borderLow rounded-lg divide-y divide-borderLow">
              {data.games.map((g, i) => (
                <div key={i} className="flex items-center gap-3 px-5 py-3">
                  ...
                  {(g.isPlayoff || g.isConsolation) && (
                    <span className="text-label text-mutedLow uppercase shrink-0 hidden sm:inline">
                      {g.isPlayoff ? 'playoff' : 'consolation'}
                    </span>
                  )}
                </div>
              ))}
